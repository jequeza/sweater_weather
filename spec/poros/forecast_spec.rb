require 'rails_helper'

RSpec.describe Forecast do
  it "exists" do
    data = {:lat=>39.7385,
 :lon=>-104.9849,
 :timezone=>"America/Denver",
 :timezone_offset=>-21600,
 :current=>
  {:dt=>1619375569,
   :sunrise=>1619352457,
   :sunset=>1619401664,
   :temp=>73.53,
   :feels_like=>71.15,
   :pressure=>1001,
   :humidity=>12,
   :dew_point=>19.44,
   :uvi=>7.98,
   :clouds=>12,
   :visibility=>10000,
   :wind_speed=>4.14,
   :wind_deg=>354,
   :wind_gust=>5.57,
   :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}]},
 :hourly=>
  [{:dt=>1619373600,
    :temp=>72.7,
    :feels_like=>70.29,
    :pressure=>1001,
    :humidity=>13,
    :dew_point=>20.52,
    :uvi=>7.43,
    :clouds=>15,
    :visibility=>10000,
    :wind_speed=>2.68,
    :wind_deg=>35,
    :wind_gust=>3.38,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619377200,
    :temp=>73.53,
    :feels_like=>71.15,
    :pressure=>1001,
    :humidity=>12,
    :dew_point=>19.44,
    :uvi=>7.98,
    :clouds=>12,
    :visibility=>10000,
    :wind_speed=>4.14,
    :wind_deg=>354,
    :wind_gust=>5.57,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619380800,
    :temp=>73.33,
    :feels_like=>70.97,
    :pressure=>1001,
    :humidity=>13,
    :dew_point=>20.97,
    :uvi=>7.43,
    :clouds=>12,
    :visibility=>10000,
    :wind_speed=>5.75,
    :wind_deg=>39,
    :wind_gust=>11.74,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619384400,
    :temp=>73.92,
    :feels_like=>71.62,
    :pressure=>999,
    :humidity=>13,
    :dew_point=>21.38,
    :uvi=>5.93,
    :clouds=>14,
    :visibility=>10000,
    :wind_speed=>4.36,
    :wind_deg=>47,
    :wind_gust=>15.23,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619388000,
    :temp=>74.93,
    :feels_like=>72.73,
    :pressure=>998,
    :humidity=>13,
    :dew_point=>22.08,
    :uvi=>4.08,
    :clouds=>21,
    :visibility=>10000,
    :wind_speed=>2.01,
    :wind_deg=>304,
    :wind_gust=>18.07,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619391600,
    :temp=>74.79,
    :feels_like=>72.57,
    :pressure=>997,
    :humidity=>13,
    :dew_point=>21.99,
    :uvi=>2.18,
    :clouds=>22,
    :visibility=>10000,
    :wind_speed=>2.98,
    :wind_deg=>314,
    :wind_gust=>19.06,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619395200,
    :temp=>74.68,
    :feels_like=>72.46,
    :pressure=>996,
    :humidity=>13,
    :dew_point=>21.49,
    :uvi=>0.86,
    :clouds=>22,
    :visibility=>10000,
    :wind_speed=>3.53,
    :wind_deg=>229,
    :wind_gust=>16.28,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619398800,
    :temp=>72.3,
    :feels_like=>69.94,
    :pressure=>996,
    :humidity=>15,
    :dew_point=>22.89,
    :uvi=>0.21,
    :clouds=>5,
    :visibility=>10000,
    :wind_speed=>4.25,
    :wind_deg=>210,
    :wind_gust=>9.55,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619402400,
    :temp=>68.23,
    :feels_like=>65.61,
    :pressure=>997,
    :humidity=>18,
    :dew_point=>23.23,
    :uvi=>0,
    :clouds=>7,
    :visibility=>10000,
    :wind_speed=>7.94,
    :wind_deg=>196,
    :wind_gust=>16.67,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619406000,
    :temp=>66.74,
    :feels_like=>63.91,
    :pressure=>999,
    :humidity=>17,
    :dew_point=>20.62,
    :uvi=>0,
    :clouds=>7,
    :visibility=>10000,
    :wind_speed=>13.98,
    :wind_deg=>203,
    :wind_gust=>24.2,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619409600,
    :temp=>64.45,
    :feels_like=>61.45,
    :pressure=>999,
    :humidity=>18,
    :dew_point=>21.04,
    :uvi=>0,
    :clouds=>5,
    :visibility=>10000,
    :wind_speed=>10.96,
    :wind_deg=>202,
    :wind_gust=>20.49,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619413200,
    :temp=>63.09,
    :feels_like=>59.99,
    :pressure=>999,
    :humidity=>19,
    :dew_point=>19.56,
    :uvi=>0,
    :clouds=>4,
    :visibility=>10000,
    :wind_speed=>9.19,
    :wind_deg=>198,
    :wind_gust=>19.82,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619416800,
    :temp=>60.26,
    :feels_like=>57.07,
    :pressure=>999,
    :humidity=>23,
    :dew_point=>21.9,
    :uvi=>0,
    :clouds=>3,
    :visibility=>10000,
    :wind_speed=>7.76,
    :wind_deg=>190,
    :wind_gust=>15.28,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619420400,
    :temp=>57.99,
    :feels_like=>54.72,
    :pressure=>999,
    :humidity=>26,
    :dew_point=>23.76,
    :uvi=>0,
    :clouds=>0,
    :visibility=>10000,
    :wind_speed=>8.39,
    :wind_deg=>180,
    :wind_gust=>17.27,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619424000,
    :temp=>57.06,
    :feels_like=>53.69,
    :pressure=>999,
    :humidity=>26,
    :dew_point=>22.46,
    :uvi=>0,
    :clouds=>0,
    :visibility=>10000,
    :wind_speed=>8.84,
    :wind_deg=>183,
    :wind_gust=>19.35,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619427600,
    :temp=>55.71,
    :feels_like=>52.16,
    :pressure=>999,
    :humidity=>25,
    :dew_point=>20.71,
    :uvi=>0,
    :clouds=>0,
    :visibility=>10000,
    :wind_speed=>6.78,
    :wind_deg=>185,
    :wind_gust=>13.42,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619431200,
    :temp=>54.68,
    :feels_like=>50.97,
    :pressure=>999,
    :humidity=>24,
    :dew_point=>19.26,
    :uvi=>0,
    :clouds=>0,
    :visibility=>10000,
    :wind_speed=>5.68,
    :wind_deg=>193,
    :wind_gust=>8.68,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619434800,
    :temp=>53.8,
    :feels_like=>50,
    :pressure=>999,
    :humidity=>24,
    :dew_point=>17.65,
    :uvi=>0,
    :clouds=>0,
    :visibility=>10000,
    :wind_speed=>5.7,
    :wind_deg=>200,
    :wind_gust=>9.26,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619438400,
    :temp=>52.65,
    :feels_like=>48.74,
    :pressure=>999,
    :humidity=>24,
    :dew_point=>16.68,
    :uvi=>0,
    :clouds=>2,
    :visibility=>10000,
    :wind_speed=>5.5,
    :wind_deg=>220,
    :wind_gust=>7.29,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01n"}],
    :pop=>0},
   {:dt=>1619442000,
    :temp=>53.19,
    :feels_like=>49.33,
    :pressure=>999,
    :humidity=>24,
    :dew_point=>17.24,
    :uvi=>0.19,
    :clouds=>0,
    :visibility=>10000,
    :wind_speed=>4.9,
    :wind_deg=>227,
    :wind_gust=>6.31,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619445600,
    :temp=>57.02,
    :feels_like=>53.37,
    :pressure=>999,
    :humidity=>20,
    :dew_point=>16.97,
    :uvi=>0.8,
    :clouds=>2,
    :visibility=>10000,
    :wind_speed=>3.65,
    :wind_deg=>227,
    :wind_gust=>5.88,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619449200,
    :temp=>61.63,
    :feels_like=>58.24,
    :pressure=>997,
    :humidity=>16,
    :dew_point=>15.15,
    :uvi=>2.05,
    :clouds=>4,
    :visibility=>10000,
    :wind_speed=>0.98,
    :wind_deg=>213,
    :wind_gust=>2.17,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619452800,
    :temp=>65.77,
    :feels_like=>62.65,
    :pressure=>996,
    :humidity=>13,
    :dew_point=>13.8,
    :uvi=>3.91,
    :clouds=>11,
    :visibility=>10000,
    :wind_speed=>3.96,
    :wind_deg=>41,
    :wind_gust=>6.76,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :pop=>0},
   {:dt=>1619456400,
    :temp=>69.03,
    :feels_like=>66.15,
    :pressure=>996,
    :humidity=>11,
    :dew_point=>12.65,
    :uvi=>5.83,
    :clouds=>29,
    :visibility=>10000,
    :wind_speed=>5.66,
    :wind_deg=>49,
    :wind_gust=>6.67,
    :weather=>[{:id=>802, :main=>"Clouds", :description=>"scattered clouds", :icon=>"03d"}],
    :pop=>0},
   {:dt=>1619460000,
    :temp=>71.87,
    :feels_like=>69.13,
    :pressure=>995,
    :humidity=>8,
    :dew_point=>9.75,
    :uvi=>7.3,
    :clouds=>40,
    :visibility=>10000,
    :wind_speed=>4.47,
    :wind_deg=>21,
    :wind_gust=>11.54,
    :weather=>[{:id=>802, :main=>"Clouds", :description=>"scattered clouds", :icon=>"03d"}],
    :pop=>0},
   {:dt=>1619463600,
    :temp=>73.47,
    :feels_like=>70.9,
    :pressure=>994,
    :humidity=>8,
    :dew_point=>9.41,
    :uvi=>7.82,
    :clouds=>99,
    :visibility=>10000,
    :wind_speed=>5.39,
    :wind_deg=>7,
    :wind_gust=>13.73,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619467200,
    :temp=>75.78,
    :feels_like=>73.38,
    :pressure=>993,
    :humidity=>7,
    :dew_point=>7.29,
    :uvi=>7.28,
    :clouds=>99,
    :visibility=>10000,
    :wind_speed=>3.56,
    :wind_deg=>313,
    :wind_gust=>17.45,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619470800,
    :temp=>77.38,
    :feels_like=>75.09,
    :pressure=>992,
    :humidity=>6,
    :dew_point=>8.06,
    :uvi=>5.81,
    :clouds=>73,
    :visibility=>10000,
    :wind_speed=>8.72,
    :wind_deg=>250,
    :wind_gust=>20.33,
    :weather=>[{:id=>803, :main=>"Clouds", :description=>"broken clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619474400,
    :temp=>76.68,
    :feels_like=>74.32,
    :pressure=>991,
    :humidity=>6,
    :dew_point=>7.23,
    :uvi=>3.62,
    :clouds=>79,
    :visibility=>10000,
    :wind_speed=>12.08,
    :wind_deg=>234,
    :wind_gust=>20.45,
    :weather=>[{:id=>803, :main=>"Clouds", :description=>"broken clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619478000,
    :temp=>75.58,
    :feels_like=>73.17,
    :pressure=>991,
    :humidity=>7,
    :dew_point=>8.92,
    :uvi=>1.95,
    :clouds=>84,
    :visibility=>10000,
    :wind_speed=>10.83,
    :wind_deg=>249,
    :wind_gust=>17.96,
    :weather=>[{:id=>803, :main=>"Clouds", :description=>"broken clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619481600,
    :temp=>73.83,
    :feels_like=>71.33,
    :pressure=>992,
    :humidity=>9,
    :dew_point=>11.32,
    :uvi=>0.78,
    :clouds=>86,
    :visibility=>10000,
    :wind_speed=>10.85,
    :wind_deg=>252,
    :wind_gust=>14.83,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619485200,
    :temp=>69.13,
    :feels_like=>66.31,
    :pressure=>994,
    :humidity=>12,
    :dew_point=>15.44,
    :uvi=>0.14,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>16.78,
    :wind_deg=>252,
    :wind_gust=>23.15,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04d"}],
    :pop=>0},
   {:dt=>1619488800,
    :temp=>66.47,
    :feels_like=>63.52,
    :pressure=>995,
    :humidity=>15,
    :dew_point=>17.28,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>15.73,
    :wind_deg=>238,
    :wind_gust=>23.67,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619492400,
    :temp=>64.94,
    :feels_like=>61.84,
    :pressure=>996,
    :humidity=>15,
    :dew_point=>15.96,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>10.11,
    :wind_deg=>245,
    :wind_gust=>20.02,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619496000,
    :temp=>62.98,
    :feels_like=>59.72,
    :pressure=>997,
    :humidity=>16,
    :dew_point=>16.07,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>8.41,
    :wind_deg=>208,
    :wind_gust=>17.4,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619499600,
    :temp=>60.82,
    :feels_like=>57.45,
    :pressure=>997,
    :humidity=>18,
    :dew_point=>17.67,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>9.13,
    :wind_deg=>197,
    :wind_gust=>16.58,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619503200,
    :temp=>59.32,
    :feels_like=>55.85,
    :pressure=>998,
    :humidity=>19,
    :dew_point=>17.53,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>6.85,
    :wind_deg=>193,
    :wind_gust=>11.52,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619506800,
    :temp=>58.06,
    :feels_like=>54.46,
    :pressure=>998,
    :humidity=>19,
    :dew_point=>16.72,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>6.87,
    :wind_deg=>187,
    :wind_gust=>16.17,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619510400,
    :temp=>56.55,
    :feels_like=>52.9,
    :pressure=>999,
    :humidity=>21,
    :dew_point=>16.95,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>8.39,
    :wind_deg=>183,
    :wind_gust=>15.3,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619514000,
    :temp=>55.22,
    :feels_like=>51.33,
    :pressure=>999,
    :humidity=>19,
    :dew_point=>13.93,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>10.38,
    :wind_deg=>173,
    :wind_gust=>20.58,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619517600,
    :temp=>54.81,
    :feels_like=>50.74,
    :pressure=>999,
    :humidity=>16,
    :dew_point=>9.7,
    :uvi=>0,
    :clouds=>100,
    :visibility=>10000,
    :wind_speed=>12.62,
    :wind_deg=>180,
    :wind_gust=>23.04,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619521200,
    :temp=>53.44,
    :feels_like=>49.33,
    :pressure=>1000,
    :humidity=>18,
    :dew_point=>11.28,
    :uvi=>0,
    :clouds=>99,
    :visibility=>10000,
    :wind_speed=>12.68,
    :wind_deg=>171,
    :wind_gust=>21.9,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619524800,
    :temp=>52.16,
    :feels_like=>47.97,
    :pressure=>999,
    :humidity=>19,
    :dew_point=>11.41,
    :uvi=>0,
    :clouds=>84,
    :visibility=>10000,
    :wind_speed=>12.3,
    :wind_deg=>174,
    :wind_gust=>22.64,
    :weather=>[{:id=>803, :main=>"Clouds", :description=>"broken clouds", :icon=>"04n"}],
    :pop=>0},
   {:dt=>1619528400,
    :temp=>51.87,
    :feels_like=>47.75,
    :pressure=>1000,
    :humidity=>21,
    :dew_point=>13.44,
    :uvi=>0.2,
    :clouds=>6,
    :visibility=>10000,
    :wind_speed=>6.22,
    :wind_deg=>222,
    :wind_gust=>11.88,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619532000,
    :temp=>53.37,
    :feels_like=>49.44,
    :pressure=>1000,
    :humidity=>22,
    :dew_point=>16.36,
    :uvi=>0.8,
    :clouds=>6,
    :visibility=>10000,
    :wind_speed=>4.32,
    :wind_deg=>319,
    :wind_gust=>9.6,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619535600,
    :temp=>55.53,
    :feels_like=>51.91,
    :pressure=>1001,
    :humidity=>24,
    :dew_point=>19.9,
    :uvi=>2.01,
    :clouds=>4,
    :visibility=>10000,
    :wind_speed=>14.36,
    :wind_deg=>352,
    :wind_gust=>18.25,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619539200,
    :temp=>57.58,
    :feels_like=>54.3,
    :pressure=>1000,
    :humidity=>27,
    :dew_point=>24.22,
    :uvi=>3.56,
    :clouds=>5,
    :visibility=>10000,
    :wind_speed=>13.96,
    :wind_deg=>353,
    :wind_gust=>17.4,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0},
   {:dt=>1619542800,
    :temp=>59.58,
    :feels_like=>56.35,
    :pressure=>1000,
    :humidity=>24,
    :dew_point=>23.59,
    :uvi=>5.28,
    :clouds=>7,
    :visibility=>10000,
    :wind_speed=>13.8,
    :wind_deg=>354,
    :wind_gust=>14.47,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :pop=>0}],
 :daily=>
  [{:dt=>1619373600,
    :sunrise=>1619352457,
    :sunset=>1619401664,
    :moonrise=>1619396220,
    :moonset=>1619350440,
    :moon_phase=>0.44,
    :temp=>{:day=>72.7, :min=>47.25, :max=>74.93, :night=>63.09, :eve=>74.68, :morn=>47.25},
    :feels_like=>{:day=>70.29, :night=>46.13, :eve=>72.46, :morn=>46.13},
    :pressure=>1001,
    :humidity=>13,
    :dew_point=>20.52,
    :wind_speed=>13.98,
    :wind_deg=>203,
    :wind_gust=>19.82,
    :weather=>[{:id=>801, :main=>"Clouds", :description=>"few clouds", :icon=>"02d"}],
    :clouds=>15,
    :pop=>0.07,
    :uvi=>7.98},
   {:dt=>1619460000,
    :sunrise=>1619438778,
    :sunset=>1619488125,
    :moonrise=>1619487240,
    :moonset=>1619438580,
    :moon_phase=>0.5,
    :temp=>{:day=>71.87, :min=>52.65, :max=>77.38, :night=>60.82, :eve=>73.83, :morn=>52.65},
    :feels_like=>{:day=>69.13, :night=>48.74, :eve=>71.33, :morn=>48.74},
    :pressure=>995,
    :humidity=>8,
    :dew_point=>9.75,
    :wind_speed=>16.78,
    :wind_deg=>252,
    :wind_gust=>17.4,
    :weather=>[{:id=>802, :main=>"Clouds", :description=>"scattered clouds", :icon=>"03d"}],
    :clouds=>40,
    :pop=>0,
    :uvi=>7.82},
   {:dt=>1619546400,
    :sunrise=>1619525100,
    :sunset=>1619574586,
    :moonrise=>1619578320,
    :moonset=>1619526900,
    :moon_phase=>0.52,
    :temp=>{:day=>61.68, :min=>43.66, :max=>62.64, :night=>43.66, :eve=>48.22, :morn=>52.16},
    :feels_like=>{:day=>58.53, :night=>47.97, :eve=>42.6, :morn=>47.97},
    :pressure=>1000,
    :humidity=>21,
    :dew_point=>21.4,
    :wind_speed=>17.18,
    :wind_deg=>20,
    :wind_gust=>17.63,
    :weather=>[{:id=>502, :main=>"Rain", :description=>"heavy intensity rain", :icon=>"10d"}],
    :clouds=>15,
    :pop=>0.92,
    :rain=>13.6,
    :uvi=>7.05},
   {:dt=>1619632800,
    :sunrise=>1619611423,
    :sunset=>1619661047,
    :moonrise=>1619669460,
    :moonset=>1619615580,
    :moon_phase=>0.56,
    :temp=>{:day=>52.93, :min=>38.26, :max=>59.4, :night=>54.54, :eve=>59.4, :morn=>38.26},
    :feels_like=>{:day=>49.91, :night=>34.36, :eve=>56.5, :morn=>34.36},
    :pressure=>1017,
    :humidity=>42,
    :dew_point=>30.36,
    :wind_speed=>10.27,
    :wind_deg=>0,
    :wind_gust=>10.47,
    :weather=>[{:id=>803, :main=>"Clouds", :description=>"broken clouds", :icon=>"04d"}],
    :clouds=>65,
    :pop=>0.76,
    :uvi=>7.72},
   {:dt=>1619719200,
    :sunrise=>1619697747,
    :sunset=>1619747508,
    :moonrise=>1619760420,
    :moonset=>1619704620,
    :moon_phase=>0.6,
    :temp=>{:day=>65.53, :min=>46.02, :max=>70.74, :night=>63.55, :eve=>70.63, :morn=>46.02},
    :feels_like=>{:day=>62.64, :night=>43.93, :eve=>68.23, :morn=>43.93},
    :pressure=>1019,
    :humidity=>18,
    :dew_point=>21.65,
    :wind_speed=>6.24,
    :wind_deg=>81,
    :wind_gust=>9.51,
    :weather=>[{:id=>800, :main=>"Clear", :description=>"clear sky", :icon=>"01d"}],
    :clouds=>0,
    :pop=>0.04,
    :uvi=>9.1},
   {:dt=>1619805600,
    :sunrise=>1619784072,
    :sunset=>1619833969,
    :moonrise=>0,
    :moonset=>1619794080,
    :moon_phase=>0.64,
    :temp=>{:day=>71.96, :min=>53.2, :max=>76.98, :night=>69.01, :eve=>76.95, :morn=>53.2},
    :feels_like=>{:day=>69.84, :night=>50.25, :eve=>75.18, :morn=>50.25},
    :pressure=>1019,
    :humidity=>21,
    :dew_point=>30.69,
    :wind_speed=>5.86,
    :wind_deg=>99,
    :wind_gust=>13.73,
    :weather=>[{:id=>802, :main=>"Clouds", :description=>"scattered clouds", :icon=>"03d"}],
    :clouds=>31,
    :pop=>0,
    :uvi=>10},
   {:dt=>1619892000,
    :sunrise=>1619870399,
    :sunset=>1619920429,
    :moonrise=>1619850900,
    :moonset=>1619884140,
    :moon_phase=>0.68,
    :temp=>{:day=>81.48, :min=>57.83, :max=>86.14, :night=>76.59, :eve=>85.96, :morn=>57.83},
    :feels_like=>{:day=>79.27, :night=>55.29, :eve=>82.63, :morn=>55.29},
    :pressure=>1009,
    :humidity=>13,
    :dew_point=>27.79,
    :wind_speed=>5.46,
    :wind_deg=>218,
    :wind_gust=>11.18,
    :weather=>[{:id=>803, :main=>"Clouds", :description=>"broken clouds", :icon=>"04d"}],
    :clouds=>79,
    :pop=>0,
    :uvi=>10},
   {:dt=>1619978400,
    :sunrise=>1619956726,
    :sunset=>1620006890,
    :moonrise=>1619940780,
    :moonset=>1619974440,
    :moon_phase=>0.71,
    :temp=>{:day=>83.37, :min=>63.7, :max=>86.09, :night=>73.53, :eve=>82.96, :morn=>63.7},
    :feels_like=>{:day=>80.64, :night=>61.12, :eve=>80.33, :morn=>61.12},
    :pressure=>998,
    :humidity=>9,
    :dew_point=>19.49,
    :wind_speed=>17,
    :wind_deg=>240,
    :wind_gust=>21.5,
    :weather=>[{:id=>804, :main=>"Clouds", :description=>"overcast clouds", :icon=>"04d"}],
    :clouds=>100,
    :pop=>0,
    :uvi=>10}]}

    forecast = Forecast.new(data)

    expect(forecast).to be_a Forecast
    expect(forecast.current_weather).to be_a CurrentWeather
    expect(forecast.current_weather.conditions).to eq('few clouds')
    expect(forecast.daily_weather).to be_an Array
    expect(forecast.daily_weather.length).to eq(5)
    expect(forecast.daily_weather.first).to be_a DailyWeather
    expect(forecast.daily_weather.first.conditions).to eq('few clouds')
    expect(forecast.daily_weather.second.conditions).to eq('scattered clouds')
    expect(forecast.daily_weather.third.conditions).to eq('heavy intensity rain')
    expect(forecast.daily_weather.fourth.conditions).to eq('broken clouds')
    expect(forecast.daily_weather.fifth.conditions).to eq('clear sky')
    expect(forecast.hourly_weather).to be_an Array
    expect(forecast.hourly_weather.length).to eq(8)
    expect(forecast.hourly_weather.first).to be_a HourlyWeather
    expect(forecast.id).to be_nil
  end
end